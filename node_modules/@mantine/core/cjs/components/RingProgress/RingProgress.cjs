...
const require_Curve = require("./Curve/Curve.cjs");
const require_get_curves = require("./get-curves/get-curves.cjs");
const require_RingProgress_module = require("./RingProgress.module.cjs");
...
const varsResolver = require_create_vars_resolver.createVarsResolver((_, { size, thickness, transitionDuration, startAngle }) => ({
	root: {
		"--rp-size": require_rem.rem(size),
		"--rp-label-offset": require_rem.rem(thickness * 2),
		"--rp-transition-duration": transitionDuration ? `${transitionDuration}ms` : void 0
	},
	svg: { "--rp-start-angle": `${startAngle}deg` }
}));
const RingProgress = require_factory.factory((_props) => {
	const props = require_use_props.useProps("RingProgress", defaultProps, _props);
	...
	const getStyles = require_use_styles.useStyles({
		name: "RingProgress",
		classes: require_RingProgress_module.default,
		props,
		className,
		style,
		classNames,
	...
	const curves = require_get_curves.getCurves({
		size,
		...
		rootColor,
		sectionGap
	}).map(({ data, sum, root, lineRoundCaps, offset }, index) => /* @__PURE__ */ (0, react.createElement)(require_Curve.Curve, {
		...data,
		key: index,
		size,
		thickness: clampedThickness,
...
RingProgress.classes = require_RingProgress_module.default;
