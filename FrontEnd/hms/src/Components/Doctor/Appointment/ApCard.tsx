...
import { IconClock, IconEmergencyBed, IconNotebook, IconPhone, IconProgress, IconUser } from '@tabler/icons-react';
import { formatDateTime } from '../../Utility/Date';
import { Tag } from 'primereact/tag';

const ApCard = ({id, patientName, patientId, phone, reason, status, notes}:any) => {
  ...
  <div className="flex text-sm items-center border-b gap-2">
     ...
     <div className="flex text-sm items-center border-b gap-2">
      <IconPhone size={24} className='text-primary-500 rounded-full bg-primary-500 text-white'/>
        <div>
             {`Phone: ${phone || "N/A"}`}
        </div>
    </div>
